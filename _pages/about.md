...
大家好！我是**付弋洋**，目前本科三年级，就读于中国人民大学计算机科学与技术（图灵实验班），辅修金融科技实验班（高礼研究院）。

我对人工智能和金融科技充满热情，自入学以来，积极参与各类科研项目和学术竞赛，曾获得**国家奖学金**、**中国人民大学学习优秀奖学金**、**三好学生**、**优秀学生骨干奖学金**等荣誉。我的科研方向涵盖了**LLM思想赋能统计机器学习**、**LLM垂直领域应用**、**AI for Education**等领域，并已有2篇学术论文被CCF-A/B类期刊/会议录用，2篇论文在投中。
...
- **在校成绩：** GPA：3.89/4.0 ，**GPA 排名：1/110** 
...
- **角色：** 共同第一作者  
- **研究背景：**
  1. 深度点过程是在连续时间域建模离散事件的关键工具，其仿真采样被广泛用于分析未来事件规律、验证模型拟合和生成大规模数据；
  2. 当前基于Transformer的深度点过程研究聚焦于提升模型拟合能力，但对采样效率的优化存在显著空白；
  3. 主流Thinning Algorithm在生成候选时间戳时需通过Transformer前向传播计算条件强度函数，且因高拒绝率导致效率低下，严重限制了实际应用场景。
- **我们的方法**
  将LLM领域新兴的推测解码引入深度点过程建模，在维持采样质量的同时加快采样速度。具体而言：
  1. 我们设计了一种新的Transformer点过程模型，建模事件发生时间间隔的条件概率密度而非发生时间的条件强度函数;
  2. 这一设计便于生成候选时间间隔，且更易将推测解码框架融入点过程采样;
  3. 我们以大参数量的Transformer点过程模型为Target模型，小参数量模型为Draft模型，用Draft模型生成候选时间间隔，并用Target模型计算其对数似然，基于拒绝采样思想计算接受率，从而实现只用Target模型的一次前向传播就能生成多个事件;
  4. 我们在多个数据集上验证了使用推测解码加速的采样分布与自回归采样分布相近，维持了采样质量，同时实现了2-6倍的加速比;
  5. 此外，我们发现用单层单头的Draft模型一次生成5-15个候选事件，就能在保持采样质量的同时实现最高加速比。
- **项目成果：**
...
- **角色：** 共同第一作者/第一发明人  
- **研究背景：**
  1. 伴随着互联网的发展，教育的宽度被不断拓展，形式不断的丰富，常规的线上网课(MOOC)也借由AI的发展有了进一步“进化”为AI-driven的线上课程(MAIC)；
  2. MAIC由AI老师、AI助教、AI同学组成（同学们可以向AI老师和AI助教提问，也可以和AI同学们互动）%，同时添加诸多真实设定（如AI同学会被根据不同的性格由不同的agent扮演，如：好奇宝宝、显眼包等），力图模拟一个真实，可交互性高的课堂环境，增加线上网课的参与度和体验感；
  3. 面对全新形式的线上课堂MAIC，既往关于MOOC的研究无法直接照搬到新系统上，因此，**如何在新的环境下进一步解决学生容易半途而废，无法坚持完成课程学习的线上网课顽疾**是一个值得十分指得深入研究的问题。
- **我们的方法**
  提供了一套**全面且系统的解决方案**：
  1. 我们通过预数据分析，发现学生的Meta Data以及课程伊始填写的调查问卷这些静态的数据均不能较好地反映和预测最终的学习情况，必须从学生的动态交互数据入手；
  2. 我们对数据进行清洗整理，形成适合的训练数据，并分别利用Prompt和微调的方法基于交互数据对学生的退课率进行预测（其中，我们对LLM进行了全面的测试，所使用者包含GLM，GPT，DeepSeek等）；
  3. 我们同时使用zero-shot和few-shot(包含正确实例和特殊情况解释)两种方式，以适应课程刚开始时，交互数据不多的情况；
  4. 在交互数据较为丰富的课程后期，我们则会选择训练BERT并通过其来对退课情况进行预测；
  5. 在完成预测之后，对于高风险的退课人群，我们利用已有学生交互数据，以AI老师/助教的身份，结合学生往日的问题以及未来课程的吸引点，使用LLM生成个性化邮件尝试将学生拉回到课堂，以解决学生半途而废的线上网课顽疾。
- **项目成果：**
  * 经过**同[面壁智能](https://modelbest.cn/)的协作**，该功能**已于清华[MAIC](https://mp.weixin.qq.com/s/CNqiPKFaOfwJGLY5O2dKWw)平台上线**，并在AB test中展现了其显著性能}
  * 相关论文已完成，并已投稿于 **CIKM 2025**（共同第一作者）；相关**专利已完成申请（第一发明人）**。
...
- **项目成果：**  
  * 相关论文已完成，并已<u>录用于<u> **TKDE 2025**（本科生作者之一）。
